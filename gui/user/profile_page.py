...
def profile():
    ...
    with st.form("update-form"):
        ...
        # --- Account Info ---
        if current_user.profile_pic and current_user.profile_pic.strip() != "":
            try:
                st.image(current_user.profile_pic, width=100, caption="Your Profile Picture")
            except Exception:
        ...
        with col3:
            new_name = st.text_input("Name", value=current_user.name or "")
        ...
        with col4:
            ...
            with col6:
                new_gender = st.selectbox("Gender", ["His 👦", "Her 👧", "Secret"], index=["His 👦", "Her 👧", "Secret"].index(current_user.gender or "Secret"))

            new_contact_num = st.text_input("Contact number", value=current_user.contact_num or "")
        ...
        if update_button:
            success, msg = User.check_update(new_password, new_name, new_contact_num)
            if success:
